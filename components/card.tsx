    ...
    return (
        ...
        >
            ...
            <View className="absolute right-5 top-5 flex flex-row items-center rounded-full bg-white/90 px-3 py-1.5">
                ...
                <Text className="ml-1 font-rubik-semibold text-xs text-primary">
                    {item.rating}
                </Text>
            </View>
            ...
            <View className="absolute inset-x-5 bottom-5 flex flex-col items-start">
                <Text
                    className="font-rubik-bold text-xl text-white"
                    numberOfLines={1}
                >
                    {item.name}
                </Text>
                <Text
                    className="font-rubik-regular text-base text-white"
                    numberOfLines={1}
                >
                    {item.address}
                </Text>
...
export const Card = ({ item, onPress }: FeaturedCardProps) => {
    return (
        <TouchableOpacity
            className="relative mt-4 w-full flex-1 rounded-lg border border-accent bg-white px-3 py-4 shadow-lg shadow-black-100/70"
            onPress={onPress}
        >
            <View className="absolute right-5 top-5 z-50 flex flex-row items-center rounded-full bg-white/90 p-1 px-2">
                <Image source={icons.star} className="size-2.5" />
                <Text className="ml-0.5 font-rubik-semibold text-xs text-primary">
                    {item.rating}
                </Text>
            </View>
            ...
            <View className="mt-2 flex flex-col">
                ...
                <Text className="font-rubik-regular text-xs text-black-100">
                    {item.address}
                </Text>
